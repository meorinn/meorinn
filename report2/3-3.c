...
void main_command(char *argv[]){
    ...
    for (int i = 0; i < alias_count; i++) {
        if (strcmp(argv[0], aliases[i].alias) == 0) {
            ...
            // Concatenate the alias command and the remaining arguments
            for (int j = 1; argv[j] != NULL; j++) {
                alias_argv[argc++] = argv[j];
            }
            alias_argv[argc] = NULL;

            // Copy the expanded alias command back to argv
    ...
    int expanded_argc;
    expand_wildcards(argv, j_argv, &expanded_argc);
    ...
    if (strcmp(j_argv[0], "cat") == 0) {
        ...
        execute_external_command(j_argv);
    }
}
